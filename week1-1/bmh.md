...
⇒ 결과값을 제공하지 않고 데이터들만 제공한다. 결과값을 해석해야함

- 비지도 학습 알고리즘은 이 데이터를 두가지 서로 다른 클러스터(데이터 묶음)로 구분 지을 수도 있다.

### 클러스터링

- 제공된 데이터들을 바탕으로 유사한 성질을 갖는 데이터들끼리 묶는 것을 의미
- 컴퓨터 프로그램은 데이터를 묶기만 할 뿐, 각각의 클러스터가 어떤 의미를 가지는 지는 알려주지 않음

### 비클러스터링

- 칵테일 파티 알고리즘 이라고 불림
- 뇌는 한 번에 하나의 소리에 집중함
- 특정 단어가 언급되면 그 목소리를 잘 들을 수 있음

⇒ `특정 단어를 듣는 순간`, 실험대상자의 뇌는 일제히 그 단어를 `말한 사람 목소리를 처리`했다.

⇒ 클러스터링과는 다른 것은 1. 특정한 목적이 있음 2. 결과적으로는 하나의 분류만 처리함

[칵테일 파티 알고리즘 부가 설명](https://www.brainmedia.co.kr/BrainReport/8989)
